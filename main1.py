...
def predict(data: HealthData):
    try:
        ...
        # Return as JSON
        return {
            "diabetes_probability": float(np.round(diabetes_prob, 4)),
            "heart_disease_probability": float(np.round(heart_prob, 4)),
            "sleep_apnea_probability": float(np.round(sleep_apnea_prob, 4)),
            "hypertension_probability": float(np.round(hypertension_prob, 4))
        }
